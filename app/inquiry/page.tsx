...
const Page = () => {
  ...
  const checklist = [
    '▣ 출간 목적이 비교적 분명하다.(예: 판매, 제한된 배포, 소장용 등)',
    '▣ 이 책을 만드는 이유를 한 문장으로 설명할 수 있다.',
    '▣ 이 책을 누가 읽을지 독자가 대략적으로라도 떠오른다.',
    '▣ 목차가 초안 수준이라도 정리되어 있다.',
    '▣ 워드 또는 한글 파일 형태로 원고가 일정 분량 이상 정리되어 있다.(블로그/브런치 등 게시글 X)',
    '▣ 단순 인쇄 제작이 아니라, 기획·편집의 도움이 필요하다고 느낀다.',
    '▣ 원고의 방향성 조정 및 수정에 대한 편집자의 개입을 수용할 수 있다.',
    '▣ 편집자의 피드백에 따라 원고 수정이 필요할 수 있음을 이해하고 있다.',
    '▣ 무료 문의의 범위와 계약 후 제공되는 서비스의 차이를 이해하고 있다.',
    '▣ 일정과 예산에 현실적이고 명확한 계획이 있다.',
  ];
  return (
    <div
          ...
          <div className="h-full w-full rounded-2xl bg-white px-8 py-8 shadow-sm">
            ...
            <ul className="block text-sm">
              {checklist.map((list, index) => (
                <li key={index} className="mb-2 pl-2 text-sm md:text-base">
                  {list}
                </li>
              ))}
            </ul>
